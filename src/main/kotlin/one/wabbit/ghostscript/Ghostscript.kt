...
sealed interface GsArg {
    ...
    data class Switch(val flag: String) : GsArg {
        init {
            require(flag.startsWith("-")) { "Switch must start with '-': $flag" }
        }
    ...
    data class ArgFile(val file: Path) : GsArg {
        override fun toCli() = listOf("@$file")
    }
    ...
    sealed interface Device : GsArg {
        ...
        data object PdfWrite : Device {
            override val name = "pdfwrite"
        }

        data object Ps2Write : Device {
            override val name = "ps2write"
        }

        data object Eps2Write : Device {
            override val name = "eps2write"
        }

        data object Png16m : Device {
            override val name = "png16m"
        }

        data object PngGray : Device {
            override val name = "pnggray"
        }

        data object PngAlpha : Device {
            override val name = "pngalpha"
        }

        data object Jpeg : Device {
            override val name = "jpeg"
        }

        data object JpegCmyk : Device {
            override val name = "jpegcmyk"
        }

        data object Tiff24nc : Device {
            override val name = "tiff24nc"
        }

        data object TiffGray : Device {
            override val name = "tiffgray"
        }

        data object TxtWrite : Device {
            override val name = "txtwrite"
        }

        data object XpsWrite : Device {
            override val name = "xpswrite"
        }

        data object DocxWrite : Device {
            override val name = "docxwrite"
        }
    ...
    /**
     * -o shorthand: emits -o spec *and* auto-adds -dBATCH/-dNOPAUSE at assembly time. We keep it
     * distinct to preserve your intent; the builder turns it into concrete flags.
     */
    ...
    data class PixelSize(val width: Int, val height: Int) : GsArg {
        init {
            require(width > 0 && height > 0) { "Pixel dimensions must be > 0" }
        }
    ...
    data class PageSizePoints(val widthPt: Int, val heightPt: Int) : GsArg {
        init {
            require(widthPt > 0 && heightPt > 0) { "Page size in points must be > 0" }
        }

        override fun toCli() =
            listOf("-dDEVICEWIDTHPOINTS=$widthPt", "-dDEVICEHEIGHTPOINTS=$heightPt")
    }
    ...
    /** ---------- General switches ---------- * */
    data object Batch : GsArg {
        override fun toCli() = listOf("-dBATCH")
    }

    data object NoPause : GsArg {
        override fun toCli() = listOf("-dNOPAUSE")
    }

    data object Quiet : GsArg {
        override fun toCli() = listOf("-q") // implies -dQUIET
    }

    data object Safer : GsArg {
        override fun toCli() = listOf("-dSAFER")
    }

    data object NoSafer : GsArg {
        override fun toCli() = listOf("-dNOSAFER")
    }

    data object NoPrompt : GsArg {
        override fun toCli() = listOf("-dNOPROMPT")
    }

    data object NoDisplay : GsArg {
        override fun toCli() = listOf("-dNODISPLAY")
    }

    data class IncludePath(val path: String) : GsArg {
        override fun toCli() = listOf("-I", path)
    }
    ...
    data object AllowPsTransparency : GsArg {
        override fun toCli() = listOf("-dALLOWPSTRANSPARENCY")
    }
    ...
    data class FirstPage(val page: Int) : GsArg {
        init {
            require(page >= 1) { "FirstPage must be >= 1" }
        }
    ...
    data class LastPage(val page: Int) : GsArg {
        init {
            require(page >= 1) { "LastPage must be >= 1" }
        }

        override fun toCli() = listOf("-dLastPage=$page")
    }

    data class PageList(val spec: String) : GsArg {
        override fun toCli() = listOf("-sPageList=$spec")
    }

    data object PdfFitPage : GsArg {
        override fun toCli() = listOf("-dPDFFitPage")
    }
    ...
    data object NoOutputFonts : GsArg {
        override fun toCli() = listOf("-dNoOutputFonts")
    }

    data object UnrollForms : GsArg {
        override fun toCli() = listOf("-dUNROLLFORMS")
    }
    ...
    data class OwnerPassword(val value: String) : GsArg {
        override fun toCli() = listOf("-sOwnerPassword=$value")
    }

    data class UserPassword(val value: String) : GsArg {
        override fun toCli() = listOf("-sUserPassword=$value")
    }

    /** Permissions bitmask; semantics depend on PDF version/rev. */
    data class Permissions(val value: Int) : GsArg {
        override fun toCli() = listOf("-dPermissions=$value")
    }

    /** Historic, still seen in some workflows: */
    data class EncryptionR(val rev: Int) : GsArg {
        override fun toCli() = listOf("-dEncryptionR=$rev")
    }

    data class KeyLength(val bits: Int) : GsArg {
        override fun toCli() = listOf("-dKeyLength=$bits")
    }
    ...
    data class FileInput(val path: Path) : Input {
        override fun toCli() = listOf(path.toString())
    }

    data object StdIn : Input {
        override fun toCli() = listOf("-")
    }
}
...
private constructor(
    ...
    fun assemble(): List<String> {
        ...
        val nonOutputNonPS =
            args.filterNot {
                it is GsArg.Device ||
                    it is GsArg.OutputFile ||
                    it is GsArg.OutputShorthand ||
                    it is GsArg.PostScript
            }

        val outSpec: GsArg.OutputFile? =
            when {
        ...
        // If user used -o shorthand, mimic gs by auto-adding -dBATCH/-dNOPAUSE unless already
        // present.
    ...
    fun requireSuccess(): ExecResult {
        ...
        throw IllegalStateException(
            buildString {
                ...
                if (timedOut) {
                    append(" (timed out after ").append(durationMs).append(" ms)")
                } else {
                    append(" (exit ")
                        .append(exitCode)
                        .append(" after ")
                ...
                append('\n')
                append("Working dir: ")
                    .append(workingDirectory?.toString() ?: "<inherit>")
                    .append('\n')
                append("Command: ").append(redactedCommand.joinToString(" ")).append('\n')
                if (stderr.isNotBlank()) {
                    append("stderr")
                        .append(if (stderrTruncated) " (truncated)" else "")
                        .append(":\n")
                    append(stderr)
                }
                if (stdout.isNotBlank()) {
                    append("\nstdout")
                        .append(if (stdoutTruncated) " (truncated)" else "")
                        .append(":\n")
                    append(stdout)
                }
            }
...
class Ghostscript(
    ...
    companion object {
        private val SECRET_PREFIXES =
            listOf("-sOwnerPassword=", "-sUserPassword=", "-sPDFPassword=")

        private fun redactArgv(argv: List<String>): List<String> =
            argv.map { a ->
                SECRET_PREFIXES.firstOrNull { a.startsWith(it) }?.let { it + "******" } ?: a
            }
        ...
        fun discoverExecutable(): Path {
            System.getProperty("ghostscript.exec")?.let {
                return Paths.get(it)
            }

            val env = System.getenv()
            // Honour GS / GSC where present (GSC is Windows-only per docs)
            listOfNotNull(env["GS"], env["GSC"]).firstOrNull()?.let {
                return Paths.get(it)
            }
            ...
            val pathext =
                if (isWindows) (env["PATHEXT"] ?: ".EXE;.BAT;.CMD").split(";") else listOf("")
            ...
            for (dir in pathDirs) {
                for (base in candidates) {
                    val tryNames =
                        if (isWindows && !base.contains('.')) pathext.map { base + it }
                        else listOf(base)
            ...
            error(
                buildString {
                    ...
                    appendLine(
                        "Searched PATH (${pathDirs.size} dirs). Candidates: ${candidates.joinToString(", ")}"
                    )
                    ...
                    appendLine(
                        "  • On Windows: C:\\Program Files\\gs\\<version>\\bin\\gswin64c.exe"
                    )
...
object Gs {
    ...
    data class OptimizeOptions(
        ...
        val forceLosslessColorImages: Boolean? =
            null, // -> AutoFilterColorImages=false, EncodeColorImages=true,
        // ColorImageFilter=/FlateEncode
        val jpegQFactor: Double? =
            null, // 0.0..1.0 via /ColorACSImageDict << /QFactor X >> setdistillerparams
    )
    ...
    fun optimizePdf(
        input: Path,
        output: Path,
        opt: OptimizeOptions = OptimizeOptions(),
    ): ExecResult {
        requireReadableFile(input, "input PDF")
        requireWritableTarget(output, "output PDF")
            ...
            newCmd(
                ...
                .apply {
                    ...
                    opt.downsampleColorDpi?.let {
                        add(
                            GsArg.DownsampleColorImages(true),
                            GsArg.ColorImageResolution(it),
                            GsArg.ColorImageDownsampleType(GsArg.DownsampleType.Bicubic),
                        )
                    }
                    opt.downsampleGrayDpi?.let {
                        add(
                            GsArg.DownsampleGrayImages(true),
                            GsArg.GrayImageResolution(it),
                            GsArg.GrayImageDownsampleType(GsArg.DownsampleType.Bicubic),
                        )
                    }
                    opt.downsampleMonoDpi?.let {
                        add(GsArg.DownsampleMonoImages(true), GsArg.MonoImageResolution(it))
                    }

                    opt.detectDuplicateImages?.let {
                        add(GsArg.DistillerBoolean("DetectDuplicateImages", it))
                    }
                    ...
                    opt.jpegQFactor?.let { q ->
                        ...
                        add(
                            GsArg.PostScript(
                                "<< /ColorACSImageDict << /QFactor $q >> >> setdistillerparams"
                            )
                        )
                    }
    ...
    /**
     * Convert/leave colors with an optional compatibility cap (e.g., "1.3" to force transparency
     * flatten).
     */
    fun convertColors(
        input: Path,
    ...
    /**
     * Create PDF/A (1|2|3). You must supply a PDFA_def.ps path that points to a valid OutputIntent
     * (ICC). NOTE: We do NOT force -dSAFER here; PDFA_def.ps typically references an ICC from disk.
    ...
    enum class EncryptionRevision(val value: Int) {
        R2(2),
        R3(3),
    }

    /** Allowed key lengths for these revisions. */
    enum class KeyLength(val bits: Int) {
        Bits40(40),
        Bits128(128),
    }
    ...
    data class EncryptOptions(
        ...
        val allowedPermissions: Set<PdfPermission> =
            setOf(
                PdfPermission.Print,
                PdfPermission.PrintHigh,
                PdfPermission.Copy,
                PdfPermission.Modify,
                PdfPermission.Annotate,
                PdfPermission.FillForms,
                PdfPermission.ExtractForAccessibility,
                PdfPermission.Assemble,
            ),
    )
    ...
    /**
     * Render pages to image files, e.g. outPattern="/tmp/page-%03d.png". Use a printf-like %d in
     * outPattern for multi-page output.
     */
    fun renderToImages(
        input: Path,
        outPattern: String,
        opt: RasterOptions = RasterOptions(),
    ): ExecResult {
        requireReadableFile(input, "input PDF")
        ...
        // No directory validation here because pattern may contain printf parts; still check parent
        // if static.
    ...
    fun extractText(
        input: Path,
        outputTxt: Path,
        mode: GsArg.TextFormatMode = GsArg.TextFormatMode.Utf8Layout,
    ): ExecResult {
        requireReadableFile(input, "input PDF")
    ...
    fun pdfToPs(input: Path, outputPs: Path): ExecResult =
        convertVector(input, outputPs, GsArg.Device.Ps2Write)

    fun pdfToEps(input: Path, outputEps: Path): ExecResult =
        convertVector(input, outputEps, GsArg.Device.Eps2Write)

    fun pdfToXps(input: Path, outputXps: Path): ExecResult =
        convertVector(input, outputXps, GsArg.Device.XpsWrite)
    ...
    sealed interface PsColor {
        data class Gray(val g: Double) : PsColor {
            init {
    ...
    sealed interface AlphaMode {
        ...
        data class Constant(val alpha: Double) : AlphaMode {
            init {
     ...
     * Transparency uses Ghostscript’s PDF 1.4 operators when available; otherwise falls back to
     * solid.
     */
    fun watermarkTextFlattened(
        ...
        val ps =
            buildTextWatermarkPS(text, font, fontSizePt, rotationDeg, color, alpha, anchor, pages)
        ...
        runCatching { Gs.countPages(outPdf) }
            .onSuccess { pagesOut ->
                check(pagesOut > 0) {
     ...
     *
     * Currently supports JPEG input reliably (embedded with DCTDecode), scaled to a target box in
     * points. If you pass PNG, convert externally or supply a JPEG for predictable results.
     */
    fun watermarkImageFlattened(
        ...
        val ps =
            buildImageWatermarkPS(
                imagePath = imageFile.toAbsolutePath().toString(),
                drawWidthPt = drawWidthPt,
                drawHeightPt = drawHeightPt,
                rotationDeg = rotationDeg,
                anchor = anchor,
                alpha = alpha,
                pages = pages,
                imgWidthPx = info.widthPx,
                imgHeightPx = info.heightPx,
    ...
    /**
     * Rotate pages by updating the /Rotate key (pdfmark/EndPage). Works on a selected subset or
     * globally, and is non-destructive to page content.
     */
    fun rotatePages(
        inPdf: Path,
        outPdf: Path,
        ...
        require(degrees in setOf(0, 90, 180, 270)) {
            "degrees must be one of 0, 90, 180, 270 (got $degrees)"
        }
        requireReadableFile(inPdf, "input PDF")
        requireWritableTarget(outPdf, "output PDF")
     ...
     *
     * Uses pdfimage* devices (raster-to-PDF, single image per page). That guarantees no selectable
     * text, no vectors, just pixels. If you want OCR text layer, switch device names to
     * pdfocr8/24/32.
     */
    fun imageOnlyPdf(
        inPdf: Path,
        ...
        // Preflight: pdfimage* devices may be absent in some builds. Throw something
        // human-readable.
        runCatching { Ghostscript().availableDevicesBlocking() }
            .onSuccess { devs ->
            ...
            .onFailure {
                // If we can't query, proceed; the underlying exec will still report a precise
                // error.
            }
        ...
        val cmd =
            GsCommand()
                ...
                .apply {
                    ...
                    downscaleFactor?.let {
                        require(it in 2..8) { "DownScaleFactor must be in 2..8 (got $it)" }
                        add(GsArg.D("DownScaleFactor", it.toString()))
                    }
                    when (background) {
                        is RasterBackground.White -> {
                            /* default */
                        }
                        is RasterBackground.Rgb -> {
                            ...
                            add(
                                GsArg.D(
                                    "BackgroundColor",
                                    String.format(Locale.ROOT, "16#%06X", rgb),
                                )
                            )
                        }
                    }
    ...
    private fun buildRotatePagesPS(deg: Int, selSpec: PageSelector): String =
        when (selSpec) {
            PageSelector.All -> "mark /Rotate $deg /PAGES pdfmark"
            else -> {
    ...
    private fun buildTextWatermarkPS(
        ...
        val placeCode =
            when (anchor) {
                ...
                is Anchor.Absolute -> "${anchor.xPt} ${anchor.yPt} translate"
    ...
    private fun readJpegInfo(imageFile: Path): JpegInfo {
        val iis =
            ImageIO.createImageInputStream(imageFile.toFile())
                ?: error(
                    "Unsupported or unreadable image for watermark: $imageFile (no ImageInputStream)"
                )
        iis.use {
            val readers = ImageIO.getImageReaders(iis)
            require(readers.hasNext()) {
                "Unsupported or unreadable image for watermark: $imageFile (no reader)"
            }
            val r = readers.next()
            try {
                r.input = iis
                ...
                require("jpeg" in fmt) {
                    "watermarkImageFlattened requires JPEG; got $fmt for $imageFile"
                }
                val w = r.getWidth(0)
                val h = r.getHeight(0)
                ...
                val colorSpace =
                    when (val cm = imageType?.colorModel) {
                        null -> "DeviceRGB"
                        else ->
                            when (cm.numColorComponents) {
                                1 -> "DeviceGray"
                                4 -> "DeviceCMYK"
                                else -> "DeviceRGB"
                            }
    ...
    private fun pagePredicate(sel: Gs.PageSelector): SelectionPS =
        when (sel) {
    ...
    /**
     * Split into one PDF per page using pdfwrite's %d pattern (no loops). Example outPattern:
     * "/tmp/invoice-%04d.pdf"
     */
    fun splitPages(
        input: Path,
        outPattern: String = input.resolveSibling("${input.stem()}-%04d.pdf").toString(),
    ): ExecResult {
        requireReadableFile(input, "input PDF")
        ...
        runCatching { Paths.get(outPattern) }
            .onSuccess { p ->
                p.parent?.let {
                    requireWritableTarget(it.resolve("dummy.tmp"), "output pattern parent")
                }
            }
    ...
    /**
     * Auto-crop to content (BBox) with optional uniform margin in points. Implementation: run bbox
     * once, parse per-page HiResBoundingBox, then apply per page via EndPage hook.
     */
    fun autoCropToContent(input: Path, output: Path, marginPt: Int = 0): ExecResult {
        ...
        val measureCmd =
            GsCommand()
                ...
                .input(GsArg.FileInput(input))
        val measure =
            Ghostscript(config = Ghostscript.RunnerConfig(maxOutputBytes = 16 * 1024 * 1024))
    ...
    fun mergeWithBookmarks(
        ...
        require(inputs.isNotEmpty()) { "inputs must not be empty" }
        require(titles.size == inputs.size) {
            "titles size (${titles.size}) must match inputs size (${inputs.size})"
        }
        requireReadableFiles(inputs, "input PDF")
        requireWritableTarget(output, "output PDF")
    ...
    /**
     * Add flattened page numbers via EndPage hook. We precompute all labels (so your formatter can
     * be arbitrary). dxPt/dyPt are margins; for Custom they are absolute (from bottom-left).
     */
    fun addPageNumbers(
        input: Path,
        output: Path,
        ...
        // pdfmarkString(...) should already exist in your helpers; if not, escape like your other
        // PS strings.
     ...
     *
     * Implementation detail: we DO NOT call PostScript `file` at all. We stream each JPEG as
     * ASCII85 to stdin and read it with `currentfile /ASCII85Decode /DCTDecode ...`. This avoids
     * SAFER/permit headaches and works on macOS (/var vs /private/var) cleanly.
     */
    fun imagesToPdf(
        images: List<Path>,
        output: Path,
            ...
            buildString(infos.size * 256) {
                ...
                for (img in infos) {
                    ...
                    val lay: Layout =
                        when (fitMode) {
                            Fit.FitToImage ->
                                Layout(imgWpt, imgHpt, imgWpt, imgHpt, 0.0, 0.0, false)
                            is Fit.InsidePaper -> {
                                val (pw, ph) =
                                    paperSizePoints(fitMode.paper)
                                        ?: error(
                                            "Unknown paper '${fitMode.paper}'. Try: letter, legal, tabloid, a3, a4, a5"
                                        )
                    ...
                    if (backgroundWhite) {
                        appendLine(
                            "gsave 1 setgray 0 0 moveto ${lay.pageW} 0 rlineto 0 ${lay.pageH} rlineto -${lay.pageW} 0 rlineto closepath fill grestore"
                        )
                    }
        ...
        return Ghostscript()
            .executeBlocking(cmd, stdinBytes = ps.toByteArray(Charsets.US_ASCII))
            .requireSuccess()
    }

    enum class AnnotationPolicy {
        FlattenAll,
        PreserveAll,
        PreserveLinksOnly,
    }
    ...
    fun normalizeAnnotations(input: Path, output: Path, policy: AnnotationPolicy): ExecResult {
        ...
        val cmd =
            GsCommand()
                ...
                .add(
                    when (policy) {
                        AnnotationPolicy.FlattenAll ->
                            GsArg.DistillerBoolean("PreserveAnnots", false)
                        AnnotationPolicy.PreserveAll ->
                            GsArg.DistillerBoolean("PreserveAnnots", true)
                        AnnotationPolicy.PreserveLinksOnly -> {
                            ...
                            // We still include the toggle while surfacing that it's a best-effort
                            // knob.
